...
extern enum nss_status _nss_llmnr_gethostbyname_r (const char *name, struct hostent * result,
			   char *buffer, size_t buflen, int *errnop,
			   int *h_errnop);

extern enum nss_status _nss_llmnr_gethostbyname2_r (const char *name, int af, struct hostent * result,
			    char *buffer, size_t buflen, int *errnop,
			    int *h_errnop);
extern enum nss_status _nss_llmnr_gethostbyaddr_r (struct in_addr * addr, int len, int type,
			   struct hostent * result, char *buffer,
			   size_t buflen, int *errnop, int *h_errnop);
extern enum nss_status _nss_llmnr4_gethostbyname_r (const char *name, struct hostent * result,
			   char *buffer, size_t buflen, int *errnop,
			   int *h_errnop);

extern enum nss_status _nss_llmnr4_gethostbyname2_r (const char *name, int af, struct hostent * result,
			    char *buffer, size_t buflen, int *errnop,
			    int *h_errnop);
extern enum nss_status _nss_llmnr4_gethostbyaddr_r (struct in_addr * addr, int len, int type,
			   struct hostent * result, char *buffer,
			   size_t buflen, int *errnop, int *h_errnop);
extern enum nss_status _nss_llmnr6_gethostbyname_r (const char *name, struct hostent * result,
			   char *buffer, size_t buflen, int *errnop,
			   int *h_errnop);

extern enum nss_status _nss_llmnr6_gethostbyname2_r (const char *name, int af, struct hostent * result,
			    char *buffer, size_t buflen, int *errnop,
			    int *h_errnop);
extern enum nss_status _nss_llmnr6_gethostbyaddr_r (struct in_addr * addr, int len, int type,
			   struct hostent * result, char *buffer,
			   size_t buflen, int *errnop, int *h_errnop);
...
__nss_bsdcompat_getaddrinfo(void *retval, void *mdata __unused, va_list ap)
{
	...
	char			*hap;
	struct hostent		*hp;
	void 			*mbufp;	/* buffer handed to llmnr */
	const char 		*name;
	...
	case AF_UNSPEC:
		status = _nss_llmnr_gethostbyname_r(name, hp, buffer, mbuflen,
						   &_errno, &_h_errno);
		break;
	case AF_INET:
		status = _nss_llmnr4_gethostbyname_r(name, hp, buffer, mbuflen,
						    &_errno, &_h_errno);
		break;
	case AF_INET6:
		status = _nss_llmnr6_gethostbyname_r(name, hp, buffer, mbuflen,
						    &_errno, &_h_errno);
		break;
	default:
		break;
...
__nss_bsdcompat_gethostbyaddr_r(void *retval, void *mdata __unused, va_list ap)
{
	...
	*resultp = NULL;
	status = _nss_llmnr_gethostbyaddr_r(addr, len, af, hp, buf, buflen,
	    &ret_errno, h_errnop);
    ...
    va_list ap)
{
	...
	*resultp = NULL;
	...
	status = _nss_llmnr_gethostbyname2_r(name, af, hp, buf, buflen,
	    &ret_errno, h_errnop);
...
__nss_bsdcompat_ghbyaddr(void *retval, void *mdata __unused, va_list ap)
{
	...
	status = _nss_llmnr_gethostbyaddr_r(src, len, af, hp, buffer,
...
__nss_bsdcompat_ghbyname(void *retval, void *mdata __unused, va_list ap)
{
	...
	status = _nss_llmnr_gethostbyname_r(name, hp, buffer, buflen, errp,
	    &h_errnop);
